...
public:
    ...
    par_task_size(const ParSize& tasks_number, const Size& max_par_tasks_number, const Size& min_tasks_per_par_task=Size{1})
    {
        const ParSize par_tasks_number = std::min(static_cast<ParSize>(max_par_tasks_number), tasks_number/static_cast<ParSize>(min_tasks_per_par_task));
        if (par_tasks_number!=0){
            par_tasks_number_ = par_tasks_number;
            par_task_size_ = tasks_number/par_tasks_number;
            rem_ = static_cast<size_type>(tasks_number%par_tasks_number);
        }
    }
    size_type size()const{
        return static_cast<size_type>(par_tasks_number_);
    }
