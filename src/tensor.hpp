...
public:        
    ...
    tensor_type operator()(slices_init_type subs)const{
        ...
        return tensor_type{view_factory<ValT,CfgT>::create_view_slice(impl(), filled_subs)};
    }
    ...
    tensor_type transpose(const Subs&...subs)const{
        ...
        return tensor_type{view_factory<ValT,CfgT>::create_view_transpose(impl(), shape_type{subs...})};
    }
    template<typename...Subs, std::enable_if_t<std::conjunction_v<std::is_convertible<Subs,index_type>...>,int> = 0 >
    tensor_type operator()(const Subs&...subs)const{
        detail::check_subdim_subs(shape(), subs...);        
        return tensor_type{view_factory<ValT,CfgT>::create_view_subdim(impl(), shape_type{subs...})};
    }            
    tensor_type operator()()const{
        detail::check_subdim_subs(shape());        
        return tensor_type{view_factory<ValT,CfgT>::create_view_subdim(impl(), shape_type{})};
    }        
    template<typename...Subs, std::enable_if_t<std::conjunction_v<std::is_convertible<Subs,index_type>...>,int> = 0 >
    tensor_type reshape(const Subs&...subs)const{
        detail::check_reshape_subs(size(), subs...);        
        return tensor_type{view_factory<ValT,CfgT>::create_view_reshape(impl(), shape_type{subs...})};
    }
