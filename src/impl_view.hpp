...
*/
template<typename ValT, template<typename> typename Cfg, typename DescT, typename ParentT>
class view_impl : public tensor_impl_base<ValT, Cfg> {
    ...
    DescT descriptor;
    std::shared_ptr<ParentT> parent;
    storage_type cache{};

public:
    template<typename DtT>
    view_impl(DtT&& descriptor_, const std::shared_ptr<ParentT>& parent_):
