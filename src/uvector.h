...
{
	static_assert(std::is_trivially_copyable<Tp>(), "uvector can hold only trivial type");
public:
	/// Element type
...
private:
	...
	/*
	* push_back if(_end == _endOfStorage) enlarge(enlarge_size(1));
	* every time extra storage is needed new capacity will be size() + std::max(size(), extra_space_needed)
	*/
	...
	/*
	* size is _end - _begin
	* incorrect if n < capacity() i.e. _end is not updated
	* else branch added
	*/
