...
using gtensor::expression_template_engine;
...
class no_dispatching_storage_engine : public expression_template_storage_engine<ValT,CfgT>
{
public:
...
class no_dispatching_engine : public expression_template_engine<ValT,CfgT,F,Ops...>
{
public:
    using expression_template_engine::expression_template_engine;
...
template<typename ValT, typename CfgT>
class dispatching_in_walker_storage_engine : public expression_template_storage_engine<ValT,CfgT>
{
public:
    using expression_template_storage_engine::expression_template_storage_engine;
    auto create_walker()const{return storage_walker<ValT,CfgT>{host()->shape(),host()->strides(),data()};}
...
template<typename ValT, typename CfgT, typename F, typename...Ops>
class dispatching_in_walker_engine : public expression_template_engine<ValT,CfgT,F,Ops...>
{
public:
    using expression_template_engine::expression_template_engine;
    template<typename...Wks>
    auto create_broadcast_walker_helper(Wks&&...walkers)const{
        return evaluating_dispatching_walker<ValT,CfgT,F,Wks...>{host()->shape(),host()->strides(), false ,std::forward<Wks>(walkers)...};
    }
    ...
    auto create_trivial_walker_helper(Wks&&...walkers)const{
        return evaluating_dispatching_walker<ValT,CfgT,F,Wks...>{host()->shape(),host()->strides(), true ,std::forward<Wks>(walkers)...};
    }
    auto create_trivial_walker()const{
        return std::apply(
...
template<typename ValT, typename CfgT>
class variant_dispatch_storage_engine : public expression_template_storage_engine<ValT,CfgT>
{
public:
...
public:
    ...
    auto create_broadcast_walker_helper(std::false_type, Wks&&...walkers)const{
        ...
        return variant_type{
            walker<ValT,CfgT>{
                std::make_unique<evaluating_walker_polymorphic_type>(evaluating_walker_type{host()->shape(),std::forward<Wks>(walkers)...})
            }
        };
    }
