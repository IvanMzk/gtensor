...
TEMPLATE_TEST_CASE("benchmark_reduce_bunary_sum","[benchmark_tensor]",
    gtensor::config::c_order,
    gtensor::config::f_order
)
{
    ...
    for (auto it=shapes.begin(), last=shapes.end(); it!=last; ++it){
        ...
        //sum
        {
            ...
            auto res = benchmark_expression_template_helpers::sum(t,axis);
            auto stop = cpu_timer{};
            std::cout<<std::endl<<"sum "<<stop-start<<" ms";
        }
    }
}
