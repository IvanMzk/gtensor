...
struct bench_iterator_helper{
    ...
    auto operator()(std::string mes, std::size_t n_iters, Shapes shapes, Builder builder, Traverser traverser, bool reverse_iterator=false){
...
template<typename Shapes, typename Builder, typename Traverser>
auto bench_iterator(std::string mes, std::size_t n_iters, Shapes shapes, Builder builder, Traverser traverser, bool reverse_iterator=false){
    using value_type = double;
    bench_iterator_helper<gtensor::tensor<value_type,c_order>,c_order>{}(mes,n_iters,shapes,builder,traverser,reverse_iterator);
    bench_iterator_helper<gtensor::tensor<value_type,c_order>,f_order>{}(mes,n_iters,shapes,builder,traverser,reverse_iterator);
    bench_iterator_helper<gtensor::tensor<value_type,f_order>,c_order>{}(mes,n_iters,shapes,builder,traverser,reverse_iterator);
    bench_iterator_helper<gtensor::tensor<value_type,f_order>,f_order>{}(mes,n_iters,shapes,builder,traverser,reverse_iterator);
}
...
TEST_CASE("benchmark_iterator","[benchmark_tensor]")
{
    ...
    //expression view
    bench_iterator("expression t+t+t+t+t+t+t+t+t+t traverse forward",n_iters,shapes,[](auto&& t){return t+t+t+t+t+t+t+t+t+t;},traverse_forward);
    //transpose view
    ...
    //reshape_view
    bench_iterator("reshape view t.reshape((2000,3000)), c_order, traverse forward",n_iters,shapes,[](auto&& t){return t.reshape({2000,3000},c_order{});},traverse_forward);
    bench_iterator("reshape view t.reshape((2000,3000)), f_order, traverse forward",n_iters,shapes,[](auto&& t){return t.reshape({2000,3000},f_order{});},traverse_forward);
    //mapping view
    bench_iterator("mapping view t(t>0), traverse forward",n_iters,shapes,[](auto&& t){return t(t>0);},traverse_forward);
    //view of view
    bench_iterator("transpose of expression t+t+t+t+t+t+t+t+t+t traverse forward",n_iters,shapes,[](auto&& t){return (t+t+t+t+t+t+t+t+t+t).transpose();},traverse_forward);
    bench_iterator("transpose of slice view t[0:-1,:,:,::-1] traverse forward",n_iters,shapes,[](auto&& t){return t({{0,-1,1},{},{},{{},{},-1}}).transpose();},traverse_forward);
}
