...
auto bench_iterator2(Layout, TraverseOrder, std::string mes, std::size_t n_iters, Shapes shapes, Builder builder_f, Traverser traverse_f, bool reverse_iterator=false){
...
auto deref_no_traverse = [](auto&& first, auto&& last){
    using iterator_type = std::remove_cv_t<std::remove_reference_t<decltype(first)>>;
    using value_type = typename std::iterator_traits<iterator_type>::value_type;
    value_type r{0};
    auto n = last-first;
...
template<typename Layout, typename TraverseOrder, typename Shapes, typename Builder, typename Traverser>
auto bench_iterator1(Layout, TraverseOrder, std::string mes, std::size_t n_iters, Shapes shapes, Builder builder_f, Traverser traverse_f, bool reverse_iterator=false){
    using value_type = double;
    bench_iterator_helper<gtensor::tensor<value_type,Layout>,TraverseOrder>{}(mes,n_iters,shapes,builder_f,traverse_f,reverse_iterator);
}
...
TEST_CASE("benchmark_iterator_small_shapes","[benchmark_tensor]")
{
    ...
    using benchmark_iterator_::deref_no_traverse;
    using gtensor::config::c_order;
    using gtensor::config::f_order;
    ...
    //view of view
    //bench_iterator("transpose of expression t+t+t+t+t+t+t+t+t+t traverse",n_iters,shapes,[](auto& t){return (t+t+t+t+t+t+t+t+t+t).transpose();},traverse_forward);
    //bench_iterator("transpose of slice view t[0:-1,:,:,::-1] traverse forward",n_iters,shapes,[](auto& t){return t({{0,-1,1},{},{},{{},{},-1}}).transpose();},traverse_forward,true);
