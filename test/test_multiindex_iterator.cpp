...
struct test_tensor : public tensor<ValT,Cfg>{    
    ...
    const expression_base* as_expression()const{return dynamic_cast<const expression_base*>(get_impl().get());}
    auto begin()const{return as_expression()->begin();}
    auto end()const{return as_expression()->end();}
};
